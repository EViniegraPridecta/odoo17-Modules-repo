...
export class appointmentDashboard extends Component {
  ...
  _onchangeAppointmentFilter(ev) {
    this.flag = 1
    var user_selection = $('#user_selection').val();
    var duration_selection = $('#request_date').val();
    this.month_wise_appointmentChart();
    this.top_demanded_service_chart();
    this.top_revenue_service_chart();
    this.render_upcoming_appointment_list(this.state.rowsPerPage, this.state.current_upcoming_appointment_page);
    ...
    var self = this;
    jsonrpc('/appointment/filter-apply', {
      'data': {
        'user': user_selection,
   ...
   downloadReport(e) {
    ...
    var opt = {
      margin: 1,
      filename: 'AppointmentDashboard.pdf',
  ...
  async month_wise_appointmentChart(ev) {
    var user_selection = $('#user_selection').val();
    var month_chart_selection = $('#month_chart_selection').val();

    
    var self = this;
    await jsonrpc("/month_wise/appointment/chart/data",
      {
        'data':
        {
        ...
        var ctx = $("#month_wise_appointment_chart_data");
        new Chart(ctx, {
  ...
  async top_demanded_service_chart(ev) {
    var user_selection = $('#user_selection').val();
    var duration_selection = $('#request_date').val();
    var service_chart_selection = $('#service_chart_selection').val();
    var service_range_selection = $('#service_range_selection').val();

    var self = this;

    await jsonrpc("/top/demanded/service/chart/data", {
      ...
      var ctx = $("#top_demanded_service_chart");
      new Chart(ctx, {
  ...
  async top_revenue_service_chart(ev) {
    var user_selection = $('#user_selection').val();
    var duration_selection = $('#request_date').val();
    var service_revenue_chart_selection = $('#service_revenue_chart_selection').val();
    var service_revenue_range_selection = $('#service_revenue_range_selection').val();

    var self = this;

    await jsonrpc("/top/revenue/service/chart/data", {
      ...
      var ctx = $("#top_revenue_by_service_chart");
      new Chart(ctx, {
  ...
  async render_upcoming_appointment_list(rowsPerPage, page) {
    var appointment_upcoming_selection = $('#appointment_upcoming_selection').val();
    var user_selection = $('#user_selection').val();


    self = this
    await jsonrpc("/upcoming/appointment/list/data", {
      'data':
      {
  ...
  render_appointment_filter() {
    jsonrpc('/appointment/all_filter').then(function (data) {
      var users = data[0]
      users?.forEach(user => {
  ...
  async getCardData() {
    var self = this;
    var data = await jsonrpc('/get/appointment/tiles/data')
