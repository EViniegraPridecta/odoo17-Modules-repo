    ...
    def action_send_whatsapp(self):
        ...
        if triger_state != None:
            ...
            for trigger in triggers:
                ...
                try:
                    import requests
                    url = f"{url}/odooDeveloperApi/sendAttachmentWithVariableForOdoo/v1/{apikey}/{secureKey}"
...
class SaleOrder(models.Model):
    ...
    def send_pdf_for_whatsapp_on_web_side_sale(self):
        ...
        if check_subscription == False:
             raise UserError("Your WhatsApp Connector plan has expired.")
...
class WhatsAppTrigger(models.Model):
    ...
    @api.depends('Whatsapp_template')
    def _compute_template_preview(self):
        for rec in self:
            if rec.Whatsapp_template:
                rec.template_preview = rec.Whatsapp_template.message_preview
            else:
    ...
    def _onchange_trigger_state(self):
        ...
        if "example" in body_item and "body_text" in body_item["example"]:
            body_text_example = body_item["example"]["body_text"]

            if isinstance(body_text_example, list) and body_text_example:
                # Case 1: [['name', 'salesorder']]
                if isinstance(body_text_example[0], list):
                    example_fields = body_text_example[0]

                # Case 2: ['name', 'amount', 'orderdate']
                else:
                    example_fields = body_text_example
...
class WhatsApptemplate(models.Model):
    ...
    def action_create_template_on_meta(self):
        ...
        url = f"{base_url}/odooDeveloperApi/createTemplate/v1/{apikey}/{secureKey}"
    ...
    @api.onchange('header_media_file', 'header_type', 'header_filename')
    def _check_header_media(self):
        for rec in self:
            if not rec.header_media_file or not rec.header_type:
                continue

            # Decode base64 → bytes
            file_bytes = base64.b64decode(rec.header_media_file)
            file_size_mb = len(file_bytes) / (1024 * 1024)

            ext = os.path.splitext(rec.header_filename or "")[1].lower()

            # ---- IMAGE ----
            if rec.header_type == "image":
                if ext not in [".png", ".jpg", ".jpeg"]:
                    raise ValidationError(
                        _("Only PNG and JPEG images are allowed.")
                    )
                if file_size_mb > 5:
                    raise ValidationError(
            ...
            # ---- VIDEO ----
                ...
                if ext != ".mp4":
                    raise ValidationError(
                        _("Only MP4 videos are allowed.")
                    )
                if file_size_mb > 16:
                    raise ValidationError(
                        _("Video size must not exceed 16 MB.")
                    )
            ...
            # ---- DOCUMENT ----
                ...
                if ext != ".pdf":
                    raise ValidationError(
                        _("Only PDF documents are allowed.")
                    )
                if file_size_mb > 100:
                    raise ValidationError(
                        _("Document size must not exceed 100 MB.")
                    )
...
class WhatsappChatMessage(models.Model):
    ...
    def send_template_message(self, partner_id, template_id):
        ...
        try:
            ...
            url = f"{base_url}/odooDeveloperApi/odooSendTemplatedToTicket/v1/{apikey}/{secureKey}"
            print("url......",url)
            payload = json.dumps({
            "orgId": org_id,
